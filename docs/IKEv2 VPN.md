...
* [导言](#导言)
* [配置 VPN 客户端](#配置-ikev2-vpn-客户端)

## 导言
先进行下面 配置指南的阅读再在 文件服务器下载相应的 文件。
...
1. 右键单击 [ikev2_config_import.cmd](https://github.com/hwdsl2/vpn-extras/releases/latest/download/ikev2_config_import.cmd) 并保存这个辅助脚本到与 `.p12` 文件 **相同的文件夹**。(若无法下载，在文件服务器里也可以进行下载地址：[ikev2_config_import,cmd](http://garen.dynv6.net/ikev2_config_import.cmd))
1. 右键单击保存的脚本，选择 **属性**。单击对话框下方的 **解除锁定**，然后单击 **确定**。
1. 右键单击保存的脚本，选择 **以管理员身份运行** 并按提示操作。本次服务器对应的 IP 地址为 ： 91.217.139.138 ,一定不要写错，写错会出现问题。
